...
flip_trip = function(trip,
    ...
    flipped_trip$lp_sij23 = trip$mp_sij23
    flipped_trip$mp_sij23 = trip$lp_sij23
...
ipaik = pick(matk, itid, itype, ix, iy, lp_sij23)
jpaik = pick(matk, jtid, jtype, jx, jy, mp_sij23)
paik = rbind_list(rename(ipaik, itid=tid, itype=ttype, ix=x, iy=y, lp_sij23=zone),
                  rename(jpaik, jtid=tid, jtype=ttype, jx=x, jy=y, mp_sij23=zone))
paik = dedup(paik, tid)
...
matk = downclass(matk)
write.csv2(matk, file="survey/temp/matkat-heha18.csv", row.names=FALSE)
taus = downclass(taus)
write.csv2(taus, file="survey/temp/tausta-heha18.csv", row.names=FALSE)
paik = downclass(paik)
write.csv2(paik, file="survey/temp/paikat-heha18.csv", row.names=FALSE)
